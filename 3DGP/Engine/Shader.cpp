...
void Shader::Init(const std::wstring& path, ShaderInfo info)
{
	// 경로를 읽어 만들어줘
	CreateVertexShader(path, "VS_Main", "vs_5_0"); // 점
	CreatePixelShader(path, "PS_Main", "ps_5_0"); // 픽셀
...
void Shader::CreateShader(const std::wstring& path, const std::string& name, const std::string& version, ComPtr<ID3DBlob>& blob, D3D12_SHADER_BYTECODE& shaderByteCode)
{
	...
	if (FAILED(::D3DCompileFromFile(path.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE // 이 파일을 읽어
		, name.c_str(), version.c_str(), compileFlag, 0, &blob, &_errBlob)))
	{
		::MessageBoxA(nullptr, "Shader Create Failed !", nullptr, MB_OK); 
	}

	shaderByteCode = { blob->GetBufferPointer(), blob->GetBufferSize() };// 된다면 해당 파일의 리소스를 지정된 곳에 보내주자
}
